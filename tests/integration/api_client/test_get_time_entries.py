...
async def test_when_get_time_entries_is_called_then_time_entries_returned():
    ...
    # Act
    result = await client.async_get_time_entries(context.user_id, start_time, end_time)
    ...
    assert result[0].task_id is not None
    ...
    assert result[0].user_id == context.user_id
    ...
    assert result[1].task_id is not None
    assert result[1].task_name == "Programming"
    assert result[1].hours == 2.5
    assert result[1].start == parse_datetime("2023-06-09T16:00:00")
    assert result[1].end == parse_datetime("2023-06-09T18:30:00")
